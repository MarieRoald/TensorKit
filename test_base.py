...
def test_unfold_inverted_by_fold(random_tensor):
    for i in range(len(random_tensor.shape)):
        assert np.array_equal(
            random_tensor,
            base.fold(base.unfold(random_tensor, i), i, random_tensor.shape),
        )
...
def test_flatten_inverted_by_unflatten(random_factors):
    ...
    unflattened_factors = base.unflatten_factors(
        base.flatten_factors(random_factors), rank, sizes
    )
...
def test_khatri_rao_associative(random_factors):
    ...
    assert np.allclose(prod1, prod2, rtol=1.0e-10)
